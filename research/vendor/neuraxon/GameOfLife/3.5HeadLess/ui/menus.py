...
def run_config_screen() -> Optional[Dict[str, Any]]:
    ...
    param_specs = [
        ("World Size", 30, 10000, 500, True, lambda x: x), 
        ("Sea Percentage", 20, 80, 55, True, lambda x: x / 100.0), 
        ("Rock Percentage", 1, 20, 10, True, lambda x: x / 100.0),
        ("Starting NxErs", 1, 10000, 500, True, lambda x: x), 
        ("Food Sources", 25, 3000, 250, True, lambda x: x), 
        ("Food Respawn", 200, 600, 400, True, lambda x: x),
        ("Start Food", 25, 200, 25, True, lambda x: float(x)), 
        ("Max Neurons", 5, 500, 100, True, lambda x: x), 
        ("Global Time Steps", 30, 90, 60, True, lambda x: x),
        # NEW v3.0: Circadian cycle length
        ("Day/Night Cycle (ticks)", 300, 4800, 300, True, lambda x: x),
