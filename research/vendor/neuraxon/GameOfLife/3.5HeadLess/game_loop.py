# Neuraxon Game of Life Game Loop v4.0
...
def GameOfLife(NxWorldSize: int = 100, NxWorldSea: float = 0.60, NxWorldRocks: float = 0.05, 
               StartingNxErs: int = 30, MaxNxErs: int = 50000, MaxFood: int = 300, 
    ...
    renderer = Renderer(world, textures, TexturesAlpha, headless=True)  ### PATCH: headless turbo
    nxers: Dict[int, NxEr] = {}
    foods: Dict[int, Food] = {}
    occupied = set()
    ...
    for i in range(StartingNxErs):
        ...
        nxers[a.id] = a
        occupied.add(a.pos)
        ### HOTFIX2: pump events every 10 NxErs to prevent OS "Not Responding"
        if i % 10 == 0:
            pygame.event.pump()
            if i % 50 == 0:
                print(f"  [INIT] Creating NxErs... {i+1}/{StartingNxErs}")
    ...
    def push_effect(kind: str, pos: Tuple[int, int]): pass  ### PATCH: no-op in turbo mode
    ...
    def restart_game_with_champions():
        ...
        while len(nxers) < StartingNxErs:
            ...
            nxers[a.id] = a
            occupied.add(a.pos)
            ### HOTFIX2: pump events during restart creation
            _restart_count += 1
            if _restart_count % 10 == 0:
                pygame.event.pump()
    ...
    ### HOTFIX3-G3: Debug timing infrastructure
    import time as _time_module
    _debug_timers = {
        'tick_total': 0.0,    # Total time in simulation ticks
        'food_decay': 0.0,    # Food/death processing
        'agent_sim': 0.0,     # Agent simulation (network + inputs)
        'interactions': 0.0,  # Movement resolution + mating
        'tick_count': 0,      # Ticks counted
        'last_print': _time_module.monotonic(),
    }
    def _debug_print_timing():
        now = _time_module.monotonic()
        elapsed = now - _debug_timers['last_print']
        if elapsed < 2.0: return  # Print every 2 seconds
        tc = max(1, _debug_timers['tick_count'])
        n_alive = _debug_timers.get('_n_alive', 0)
        tps = tc / max(0.001, elapsed)
        print(f"  [PERF] {tps:.1f} ticks/s | {n_alive} alive | "
              f"per-tick: total={_debug_timers['tick_total']/tc*1000:.1f}ms "
              f"food={_debug_timers['food_decay']/tc*1000:.1f}ms "
              f"agents={_debug_timers['agent_sim']/tc*1000:.1f}ms "
              f"interact={_debug_timers['interactions']/tc*1000:.1f}ms")
        for k in ('tick_total', 'food_decay', 'agent_sim', 'interactions'):
            _debug_timers[k] = 0.0
        _debug_timers['tick_count'] = 0
        _debug_timers['last_print'] = now

    _t_tick_start = _time_module.monotonic()  # safe default
    _t_food_end = _t_tick_start
    _t_agent_end = _t_tick_start
    _turbo_gui_last_time = [0.0]  ### HOTFIX: mutable container for GUI refresh timing
    _turbo_ticks_counter = [0]    ### HOTFIX: tick counter for TPS display
    ...
    try:
        while running:
            frame_dt = renderer.tick(0)  ### HOTFIX: no vsync block — return immediately
            # accumulator += frame_dt  ### PATCH: removed — turbo mode uses time-budget
            ...
            for ev in pygame.event.get():
                if ev.type == pygame.QUIT: running = False
                    ...
                    # ### PATCH: V key disabled in turbo mode (no visual map)
                    ...
                    if btn == "playpause":
                        ...
                        if dbtn and renderer.selected_nxer_id is not None and renderer.selected_nxer_id in nxers:
                            ...
                            if clicked_name:
                                ...
                                renderer.clear_detail()  ### PATCH: no map click, leaderboard only
            ...
            # renderer.handle_input(frame_dt)  ### PATCH: no camera in turbo mode
            
            ### HOTFIX2: Adaptive tick budget — measure first tick, cap the rest
            steps_to_process = 0
            _tick_budget_start = time.monotonic()
            if not paused and not game_over:
                _n_alive = sum(1 for _a in nxers.values() if _a.alive)
                # With many agents, a single tick is expensive. Estimate budget.
                # ~0.15ms per agent per tick. Budget = 25ms for sim, rest for events/render.
                _est_tick_ms = max(0.5, _n_alive * 0.15)
                _max_ticks = max(1, int(25.0 / _est_tick_ms))
                steps_to_process = min(_max_ticks, 500)  # Adaptive cap

            for _step_idx in range(steps_to_process):
                step_tick += 1
                _t_tick_start = _time_module.monotonic()
                # ### PATCH: 104O-2 — throttle logger to every 50 ticks
                if step_tick % 50 == 0:
                    data_logger.log_tick(step_tick, nxers, full_analytics=(step_tick % 200 == 0))
                # ### PATCH: effects list no longer filtered (push_effect is no-op)
                ...
                # ### PATCH: 104O-2 — use _food_grid spatial index
                _food_grid.clear()
                for f in foods.values():
                    if f.alive:
                        _food_grid.insert(f.id, f.pos)
                food_at = _food_grid._pos_to_id
                _t_food_end = _time_module.monotonic(); _debug_timers['food_decay'] += _t_food_end - _t_tick_start
                ...
                for a in nxers.values():
                    ...
                    # =========================================================
                    if getattr(a.net.params, 'temperature_enabled', True) and step_tick % 3 == 0:  ### HOTFIX3-G5: temp every 3 ticks
                        nearby_count = count_nearby_nxers(a.pos) if step_tick % 5 == 0 else getattr(a, '_cached_nearby', 0); a._cached_nearby = nearby_count  ### HOTFIX3-G6
                        ...
                        # ### PATCH: 104O-2 — store temp modifier at NxEr level (avoids per-neuron loop)
                        a._temp_spontaneous_mod = temp_effects['spontaneous_mod']
                    ...
                    if a.tick_accum >= a.ticks_per_action:
                        ...
                        steps_to_sim = 1  ### HOTFIX3-G1: was max(1, sim_steps//GTS)≈3. Cap=1 for 3× speedup. Network still learns via plasticity.
                        ...
                        if hasattr(a.net, 'neuromod_system'):
                            ### HOTFIX2: single-pass astrocyte computation
                            _astro_sum = 0.0; _astro_cnt = 0
                            for _n in a.net.all_neurons:
                                if _n.is_active:
                                    _astro_sum += getattr(_n, 'astrocyte_state', 0.0)
                                    _astro_cnt += 1
                            a.astrocyte_activity = _astro_sum / max(1, _astro_cnt)
                        ...
                        if energy_status:
                            ...
                            if energy_status.get('efficiency', 0) > 0.5 and step_tick % 20 == 0:
                                data_logger.log_plasticity_event(step_tick, 'activity', -1, a.id, energy_status.get('efficiency', 0))  ### HOTFIX2: throttle
                        
                        # UPDATED v3.1: 6 outputs
                        o = (outs + [0, 0, 0, 0, 0, 0])[:6]
                        a.last_outputs = tuple(o)
                        if step_tick % 10 == 0: data_logger.log_io_pattern(step_tick, a.id, a.last_inputs, tuple(a.last_outputs))  ### HOTFIX2: throttle IO logging
                        O1, O2, O3, O4, O5, O6 = o
                        ...
                        if prop is not None:
                            ...
                            if prop.should_force_turn(brain_heading, force_threshold):
                                ...
                                # ### PATCH: 104O-2 — throttle proprioceptron logging ### HOTFIX3-G4
                                if step_tick % 50 == 0:
                ...
                _t_agent_end = _time_module.monotonic(); _debug_timers['agent_sim'] += _t_agent_end - _t_food_end
                ...
                _t_tick_end = _time_module.monotonic()
                _debug_timers['interactions'] += _t_tick_end - _t_agent_end
                _debug_timers['tick_total'] += _t_tick_end - _t_tick_start
                _debug_timers['tick_count'] += 1
                _debug_timers['_n_alive'] = sum(1 for _da in nxers.values() if _da.alive)
                _debug_print_timing()

                ### HOTFIX2: Break EVERY tick if budget exceeded — critical for many agents
                _tick_elapsed = time.monotonic() - _tick_budget_start
                if _tick_elapsed > 0.025:
                    steps_to_process = _step_idx + 1  # Record actual ticks done for TPS
                    break

            ### PATCH: 104O-2 TURBO — alive_count for game-over; GUI refresh 1Hz
            alive_count = sum(1 for a in nxers.values() if a.alive)
            
            # ### PATCH: Throttle circadian logging to every 500 ticks
            if alive_count > 0 and step_tick % 500 == 0:
            ...
            ### HOTFIX: GUI refresh — 1Hz, uses simple mutable counter
            _turbo_ticks_counter[0] += steps_to_process
            _now_time = time.monotonic()
            if (_now_time - _turbo_gui_last_time[0]) >= 1.0 or game_over or paused:
                _elapsed = max(0.001, _now_time - _turbo_gui_last_time[0])
                _tps = _turbo_ticks_counter[0] / _elapsed
                _turbo_gui_last_time[0] = _now_time
                _turbo_ticks_counter[0] = 0
                best_scores = {}
                ...
                renderer.draw_world(foods, nxers, rankings(), alive_count, deaths_count, births_count, paused, effects, step_tick, GlobalTimeSteps, game_over, game_index, best_scores, ticks_per_sec=_tps)

            # ### PATCH: throttle NxEr stats update to every 100 ticks
            if step_tick % 100 == 0:
                for a in nxers.values():
                    data_logger.update_nxer_stats(a)
