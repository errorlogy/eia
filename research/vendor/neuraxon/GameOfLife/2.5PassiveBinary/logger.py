...
class DataLogger:
    ...
    def _init_level2_data(self):
        self.time_series = {
            ...
            # NEW: Binary state distributions
            'active_fraction': [],
            'inactive_fraction': [],
    ...
    def _log_nxer_individual(self, tick: int, a: 'NxEr'):
        ...
        if not a.alive:
            ...
            for key in ['network_activity', 'branching_ratio', 'total_energy', 'average_energy',
                    ...
                    'active_fraction', 'inactive_fraction',
        ...
        # Network activity
        activity = sum(n.binary_state for n in active_neurons) / len(active_neurons)
        ...
        series['active_fraction'].append(sum(1 for s in states if s == 1) / len(states))
        series['inactive_fraction'].append(sum(1 for s in states if s == 0) / len(states))
    ...
    def log_tick(self, tick: int, nxers: dict = None):
        ...
        if alive_nxers:
            ...
            if all_active_neurons:
                activity = sum(n.binary_state for n in all_active_neurons) / len(all_active_neurons)
    ...
    def _log_tick_level2(self, tick: int, alive_nxers: list):
        ...
        # === BINARY STATE DISTRIBUTIONS ===
        states = [n.binary_state for n in all_active_neurons]
        active_frac = sum(1 for s in states if s == 1) / len(states)
        inactive_frac = sum(1 for s in states if s == 0) / len(states)
        self.time_series['active_fraction'].append(active_frac)
        self.time_series['inactive_fraction'].append(inactive_frac)
        ...
        for n in all_active_neurons:
            ...
            theta_eff = n.firing_threshold - threshold_mod + autoreceptor_mod
            theta_exc_effectives.append(theta_eff)
            theta_inh_effectives.append(theta_eff)
        ...
        for n in all_active_neurons:
            if n.binary_state == 0:
                theta = n.firing_threshold
                
                if n.membrane_potential > theta * 0.8:
...
def generate_paper_validation_report(logger: DataLogger) -> dict:
    ...
    report = {
        'paper_section_1_trinary_neuromodulation': {
            'binary_state_distributions': {
                'active_samples': len([x for x in logger.time_series.get('active_fraction', []) if x > 0]),
                'inactive_samples': len([x for x in logger.time_series.get('inactive_fraction', []) if x > 0]),
            },
