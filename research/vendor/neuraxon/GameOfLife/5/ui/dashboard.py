# Neuraxon Game of Life v.5.05 ui dashboard (Research Version):(Multi - Neuraxon 2.0 Compliant) Internal version 190
# Based on the Papers:
...
"""
ui/dashboard.py  (NEW in v146 / v4.84)
=======================================
...
PANEL_PAD = 12
ROW_COUNT = 6   # v180 — 11 panels (M1-M10 + Mg general-factor); 6×2 grid
COL_COUNT = 2
MIN_WIDTH = 1280
MIN_HEIGHT = 880
...
PANEL_DESCRIPTORS: List[dict] = [
    ...
    {
        # v180 — population general-factor (g) evolution. Primary = PC1
        # fraction (target ≈ 0.27, the human-comparable value reported in
        # an upcoming paper); secondaries = positive-manifold fraction and
        # mean off-diagonal r (sign matters — must be positive for a real
        # manifold). This is the headline g signature to watch live.
        'panel_id': 'Mg',
        'title': 'Mg — General factor (g)',
        'paper_ref': 'g-factor apparatus (v179+); see G_FACTOR_METHODOLOGY.md',
        'subtitle': 'PC1 frac (human ref ≈0.27). manifold/meanR must be +.',
        'metric_key': 'g_pc1_fraction',
        'metric_secondary_keys': ['g_positive_manifold', 'g_mean_offdiag_r'],
        'metric_secondary_labels': ['manifold', 'meanR'],
        'metric_secondary_colours': [(150, 230, 180), (230, 190, 140)],
        'healthy_band_key': None,
        'fmt': '.3f',
        'value_label': 'PC1',
        'spark_colour': (140, 220, 240),
    },
    {
        # v181 — g dominance / unidimensionality indicator. λ1/λ2 is the
        # ratio of the first two eigenvalues of the ability correlation
        # matrix: ≈1 ⇒ leading factors co-equal ⇒ NO unitary g (an
        # upcoming paper's headline cross-battery result); ≫1 ⇒ a genuine
        # dominant general factor. Reads the already-logged
        # g_lambda1_over_lambda2 series (no new computation, no dynamics
        # impact) — this is the single most diagnostic "did we get g?"
        # number, so it earns the 12th panel and fills the empty slot.
        'panel_id': 'Mλ',
        'title': 'Mλ — g dominance (λ1/λ2)',
        'paper_ref': 'g-factor apparatus; unidimensionality indicator',
        'subtitle': '≈1 ⇒ no unitary g.  ≫1 ⇒ real dominant factor.',
        'metric_key': 'g_lambda1_over_lambda2',
        'metric_secondary_keys': ['g_pc1_fraction'],
        'metric_secondary_labels': ['PC1'],
        'metric_secondary_colours': [(140, 220, 240)],
        'healthy_band_key': None,
        'fmt': '.2f',
        'value_label': 'λ1/λ2',
        'spark_colour': (210, 180, 250),
    },
]
...
class MetricsDashboard:
    ...
    def __init__(self, screen: pygame.Surface):
        ...
        # v158 (v4.84) — VIEW SELECTOR (ComboBox)
        # =======================================
    ...
    def _save_metrics_txt(self, logger, game_id: str) -> Optional[str]:
        """v155 (v4.84) — thin wrapper around DataLogger.save_key_metrics().
    ...
    def _draw_per_nxer_view(self, outer: pygame.Rect, logger, nxer):
        """v158 (v4.84) — Per-NxEr dashboard view. Replaces the
        ...
        panel_specs = [
            ...
            {'title': 'Fitness score',
             ...
             'unit': 'score',
             'sub': f"g(self)={getattr(nxer, '_g_score', getattr(stats, 'g_factor', 0.0)):+.3f}",  # v180
            ...
            {'title': 'General factor g',
             'value': f"{(pnts.get('g_factor', [0.0]) or [0.0])[-1]:+.3f}",
             'unit': 'g(self)',
             'sub': "PC1 factor score over this NxEr's life (v181)",
             'series': pnts.get('g_factor', []),
             'series_label': 'g',
             'color_value': (140, 220, 240)},
        ]
    ...
    def _draw_header(self, outer: pygame.Rect, logger, game_id: str):
        title_surf = self._font_h1.render(
            "Neuraxon Game of Life v 5.0 — Realtime Metrics Dashboard",
            True, TEXT_HEADER)
    ...
    def _draw_panel(self, rect: pygame.Rect, desc: dict):
        ...
        # Title (big) + ref/subtitle on the SAME line (v183 — user
        # request: the small meta text sits to the RIGHT of the big
        # title, not stacked below it, so the whole header is a single
        # line and all box content moves up).
        title = self._font_h2.render(desc['title'], True, TEXT_HEADER)
        title_y = rect.y + 8
        self.screen.blit(title, (rect.x + 12, title_y))
        meta = str(desc.get('paper_ref', ''))
        sub = str(desc.get('subtitle', ''))
        if sub:
            meta = f"{meta}  ·  {sub}" if meta else sub
        meta_x = rect.x + 12 + title.get_width() + 14
        meta_avail = rect.right - 12 - meta_x
        if meta and meta_avail > 30:
            m = meta
            if self._font_tiny.size(m)[0] > meta_avail:
                while m and self._font_tiny.size(m + '…')[0] > meta_avail:
                    m = m[:-1]
                m = (m.rstrip() + '…') if m else ''
            ms = self._font_tiny.render(m, True, TEXT_DIM)
            # Vertically centre the small text on the big title baseline.
            self.screen.blit(ms, (meta_x, title_y + max(
                0, (title.get_height() - ms.get_height()) // 2)))
        # Header is now ONE line → value sits just below the title.
        value_y = title_y + title.get_height() + 8
        ...
        # v181 — widen the value column (0.32→0.36) so wide values like
        # "0.3386" never collide with the sparkline, and reserve an 18px
        # strip BELOW the sparkline for the tick-range label so it is
        # never drawn on top of the line / out of the box.
        left_w = int(rect.width * 0.36)
        spark_x = rect.x + left_w + 4
        spark_w = max(40, rect.right - spark_x - 12)
        # v183 — header is now a single line; start the sparkline just
        # below it (was a fixed rect.y+60 sized for the old 3-line header).
        spark_y = value_y
        _LABEL_STRIP = 16
        spark_h = max(24, rect.height - spark_y + rect.y - 12 - _LABEL_STRIP)
        ...
        if band is not None:
            ...
            self.screen.blit(rsurf, (rect.x + 14, _tag_y + 14))

        # v182 — the subtitle now lives in the header (top of the box) in
        # small text, so the old bottom-of-column subtitle (which spilled
        # into the next panel) is removed entirely.
        ...
        # Tick range in the reserved strip BELOW the sparkline (v181 — was
        # drawn inside the sparkline, overlapping the line and spilling
        # toward the value column / out of the box).
        if self._cached_ticks:
            ...
            ty = min(spark_rect.bottom + 2,
                     rect.bottom - ts_surf.get_height() - 3)
            self.screen.blit(ts_surf, (spark_rect.x + 2, ty))

        # Secondary legend (top-right of sparkline) on a dark chip so it
        # stays readable and visibly contained (v181 — was rendered
        # straight onto the line and looked like overflow).
        leg_items = []
        for sk, sl, sc in zip(reversed(desc.get('metric_secondary_keys', [])),
            ...
            ss = self._cached_series.get(sk, [])
            if ss:
                leg_items.append((f"{sl}: {ss[-1]:.3f}", sc))
        if leg_items:
            pad = 5
            surfs = [(self._font_tiny.render(t, True, c), c)
                     for t, c in leg_items]
            total_w = sum(s.get_width() for s, _ in surfs) \
                + 10 * (len(surfs) - 1)
            total_w = min(total_w, spark_rect.width - 8)
            chip = pygame.Rect(spark_rect.right - total_w - 2 * pad,
                               spark_rect.y + 2,
                               total_w + 2 * pad,
                               surfs[0][0].get_height() + 2 * pad - 2)
            chip_s = pygame.Surface((chip.width, chip.height),
                                    pygame.SRCALPHA)
            chip_s.fill((12, 14, 20, 195))
            self.screen.blit(chip_s, (chip.x, chip.y))
            lx = chip.x + pad
            for s, _c in surfs:
                self.screen.blit(s, (lx, chip.y + pad - 1))
                lx += s.get_width() + 10
    ...
    def _wrap_text_capped(self, text: str, x: int, y: int, width: int,
                          colour: Tuple[int, int, int],
                          font: pygame.font.Font,
                          max_lines: int = 2) -> int:
        """v182 — word-wrap capped at max_lines; the last line is
        ellipsised if the text doesn't fit. Returns the y just below the
        last rendered line (so the caller can place the value beneath the
        header without overlap / overflow)."""
        if not text:
            return y
        ...
        if len(lines) < max_lines and line:
            lines.append(line)
        # If text was truncated, ellipsise the final line to fit width.
        consumed = sum(len(s.split(' ')) for s in lines)
        ...
        cur_y = y
        for s in lines:
            self.screen.blit(font.render(s, True, colour), (x, cur_y))
            cur_y += font.get_linesize()
        return cur_y
