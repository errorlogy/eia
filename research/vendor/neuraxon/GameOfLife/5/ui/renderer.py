# Neuraxon Game of Life v.5.05 ui renderer (Research Version):(Multi - Neuraxon 2.0 Compliant) Internal version 190
# Based on the Papers:
...
class Renderer:
    ...
    def __init__(self, world: 'World', textures: Dict[str, Optional[str]], textures_alpha: float):
        pygame.init()
        pygame.display.set_caption("Neuraxon Game of Life v 5.0 (Research Version) - By David Vivancos & Dr Jose Sanchez for Qubic Science")
    ...
    def draw_world(self, foods: Dict[int, 'Food'], nxers: Dict[int, 'NxEr'], hud: Dict[str, List[Tuple[str, str]]], alive_count: int, dead_count: int, born_count: int, paused: bool, effects: List[dict], step_tick: int, GlobalTimeSteps: int, game_over: bool, game_index: int, best_scores: Optional[Dict[str, float]] = None):
        ...
        if paused and self.selected_nxer_id is not None and self.selected_nxer_id in nxers:
            ...
            # v180 — per-NxEr g-factor (PC1 factor score) + population g context.
            _g = getattr(a, '_g_score', getattr(a.stats, 'g_factor', 0.0))
            try:
                from logger import get_data_logger as _gdl180
                _gpop = getattr(_gdl180(), '_latest_g', {}) or {}
            except Exception:
                _gpop = {}
            if _gpop:
                facts.append(
                    f"g(self): {_g:+.3f}  | pop PC1={_gpop.get('g_pc1_fraction',0):.2f} "
                    f"manifold={_gpop.get('g_positive_manifold',0):.2f} "
                    f"meanR={_gpop.get('g_mean_offdiag_r',0):+.2f}")
            else:
                facts.append(f"g(self): {_g:+.3f}  (population g pending)")
