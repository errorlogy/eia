# Neuraxon Game of Life v.5.05 components (Research Version):(Multi - Neuraxon 2.0 Compliant) Internal version 190
# Based on the Papers:
...
class Synapse:
    ...
    def calculate_delta_w(self, pre_state: int, post_state: int, neuromodulators: Dict[str, float], dt: float, receptor_activations: Dict = None, tick: int = 0, _ra_cache=None) -> float:
        ...
        if self._symmetric_stdp:
            ...
            # v178 (v4.86) BUGFIX — post_trace was initialised to 0.0 and ONLY
            # ever decayed (line ~224, fast path only); it was NEVER incremented
            # from post_state anywhere in the codebase. So the Hebbian
            # coincidence term `pre_trace * post_trace` in the LTP / LTP_neg
            # branches was identically 0 for the entire life of every synapse
            # — the paper's core Hebbian STDP (Δw = η·A⁺·I₁,₁) never fired;
            # only the dopamine-gated pre-only term contributed. The v169
            # comment above ("the post_trace tracks SIGNED post_state too")
            # describes the intended behaviour that was never implemented.
            # Mirror the pre_trace update: signed in symmetric mode.
            self.post_trace += (-self.post_trace / self.tau_ltp + float(post_state)) * dt
            ...
            self.post_trace = max(-2.0, min(2.0, self.post_trace))
        else:
            ...
            # v178 (v4.86) BUGFIX — see above. Asymmetric mode mirrors the
            # pre_trace convention: indicator-on-+1. Without this the Hebbian
            # floor (`hebbian_component`, extensively documented in the v112
            # LTP comments) multiplied post_trace≡0 and contributed nothing.
        ...
        # v152 (v4.60) STEP 3 — PLASTICITY BRAKE ON INPUT SATURATION
        # ---------------------------------------------------------
        # When the population's input neurons are saturating
        # (input_saturation_fraction > 0.5), continued STDP/LTP would
        # only entrench the runaway pattern — driving more synapses
        # over threshold, increasing saturation further. Apply a
        # brake to learning_rate_mod proportional to saturation level.
        # The logger maintains the latest population saturation reading.
        try:
            sat = get_data_logger()._latest_input_saturation
            if sat > 0.5:
                # Scale learning rate down: at sat=0.5, scale=1.0;
                # at sat=1.0, scale=0.1. So when fully saturated,
                # plasticity is 90% suppressed, allowing the existing
                # synaptic weights to relax under the homeostatic
                # rules without further potentiation.
                sat_brake = max(0.1, 1.0 - 1.8 * (sat - 0.5))
                self.learning_rate_mod *= sat_brake
        except Exception:
            pass
        ...
        # _pending_delta_w = 0 return, preserving v161-v168 behaviour exactly.
            ...
            return delta
        elif self._symmetric_stdp and pre_state == -1 and post_state >= 0:
            # v178 (v4.86) BUGFIX — was `post_state == 1`, so the (-1, 0) case
            # fell through to Δw=0 while its mirror (+1, 0) DID depress (the
            # asymmetric branch is `pre==1 and post<=0`, covering post=0). The
            # "symmetric" rule was therefore asymmetric: the +1 pathway was
            # penalised against a neutral partner but the -1 pathway was not.
            # `post_state >= 0` now covers BOTH (-1, +1) and (-1, 0) — the true
            # mirror of the asymmetric `pre==1 and post<=0` (covers (1,-1)+(1,0)).
            # Scale mirrors the asymmetric LTD: strong opposite-sign (-1,+1)
            # uses the inhib scale; neutral partner (-1,0) uses the neutral
            # scale — exactly mirroring `_ltd_inhib_scale if post==-1 else
            # _ltd_neutral_scale` in the (+1, <=0) branch.
            ach_forgetting_mult = 1.5 if ach > 0.6 else 1.0
            ltd_scale = (self._ltd_inhib_scale if post_state == 1
                         else self._ltd_neutral_scale) * ach_forgetting_mult
...
class ITUCircle:
    ...
    def compute_fitness(self, network_activity: List[float], temporal_sync: float, energy_efficiency: float) -> float:
        ...
        # v185 (v5.0) MEMORY FIX — bound fitness_history. Only the last 10
        # entries are ever consumed (see prune_unfit_neurons line ~698:
        # np.mean(self.fitness_history[-10:])). Prior to v185 this list
        # grew once per fitness compute forever, which in a multi-day NAS
        # trial or research run accumulated 10^6+ floats per ITU circle.
        # Keep a comfortable buffer (200) so any future widening of the
        # tail window still works without code changes elsewhere.
        if len(self.fitness_history) > 200:
            del self.fitness_history[:100]
        return fitness
