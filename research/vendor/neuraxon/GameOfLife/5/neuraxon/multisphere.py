# Neuraxon Game of Life v.5.05 multisphere (Research Version):(Multi - Neuraxon 2.0 Compliant) Internal version 190
# Based on the Papers:
...
# v177 (v4.85) — STRUCTURAL FIELDS that are intentionally different per
# Sphere. Everything NOT in this set is inherited verbatim from the wired
# parent NetworkParameters via deepcopy (see _derive_sphere_params).
_SPHERE_STRUCTURAL_FIELDS = frozenset({
    'network_name',
    'num_input_neurons',
    'num_hidden_neurons',
    'num_output_neurons',
...
def _derive_sphere_params(parent: NetworkParameters, *,
                          network_name: str,
                          num_input_neurons: int,
                          num_hidden_neurons: int,
                          num_output_neurons: int) -> NetworkParameters:
    """v177 (v4.85) — CRITICAL BUGFIX (the v175 bug, one layer deeper).

    Pre-v177 `build_default_multisphere` constructed each Sphere's
    NetworkParameters fresh and then copied only a 12-attribute ALLOWLIST
    from the parent. That allowlist did NOT include the architecture-level
    neural knobs `refractory_period_ticks` (v171), `post_spike_mp_reset`
    (v172) or `symmetric_stdp` (v169). v175 fixed game_loop/genetics so the
    architecture reaches the *parent* `p`, but `make_nxer()` /
    `create_offspring()` then funnel `p` through this builder, whose
    allowlist silently dropped those three params back to the config.py
    defaults (0 / 0.0 / False) for every Sphere's neurons — exactly the
    same dead-parameter pathology v175 thought it had eliminated. The
    motor-sphere hidden layer the v176 diagnostic samples therefore ran
    with refractory=0 even when the architecture (and `nas_best.json`)
    specified refractory=8, which is why hidden state-0 stayed ~0.4%.

    This helper removes the allowlist failure mode permanently: start from
    a deepcopy of the (already correctly wired) parent params — so EVERY
    behavioural parameter, including any added in the future, propagates
    automatically — then override ONLY the per-Sphere STRUCTURAL fields
    (name + neuron counts), which are the only things that legitimately
    differ between Spheres. No future neural parameter can ever silently
    fail to reach the Sphere neurons again.
    """
    sp = copy.deepcopy(parent)
    sp.network_name = network_name
    sp.num_input_neurons = int(num_input_neurons)
    sp.num_hidden_neurons = int(num_hidden_neurons)
    sp.num_output_neurons = int(num_output_neurons)
    return sp
...
def build_default_multisphere(params: NetworkParameters, 
    ...
    The default 3-sphere hierarchy (v4.5: 10 inputs / 7 outputs):
    - Sensory Sphere (Layer 0): Receives environmental inputs (10 channels, incl. Song)
    - Association Sphere (Layer 1): Integrates, learns context
    - Motor Sphere (Layer 2): Produces behavioral outputs (7 channels, incl. Sing)
    ...
    # --- Sensory Sphere ---
    # v177 (v4.85) — deepcopy parent (all behavioural params, incl.
    # refractory_period_ticks/post_spike_mp_reset/symmetric_stdp + future
    # ones) then override only structural fields. Replaces the pre-v177
    # 12-attr allowlist that silently dropped the v169-v174 neural knobs.
    sensory_params = _derive_sphere_params(
        params,
        network_name="Sensory Sphere",
        num_input_neurons=params.num_input_neurons,   # v4.5: 10 environmental inputs (incl. Song)
    ...
    brain.add_sphere(
        ...
        description="Sensory cortex-like sphere receiving 10-channel environmental input",
    )

    # --- Association Sphere ---
    assoc_params = _derive_sphere_params(
        params,
    ...
    # --- Motor Sphere ---
    motor_params = _derive_sphere_params(
        params,
        network_name="Motor Sphere",
        num_input_neurons=6,  # From association relay + direct proprioception
        num_hidden_neurons=max(3, params.num_hidden_neurons // 2),
        num_output_neurons=params.num_output_neurons,  # v4.5: 7 behavioral outputs (incl. Sing)
    )
    ...
    brain.add_sphere(
        ...
        description="Motor cortex-like sphere producing 7-channel behavioral output",
    )
...
# =============================================================================
# v179 (v4.87) — CHC SIX-SPHERE TOPOLOGY
# =============================================================================

def build_chc_multisphere(params: NetworkParameters) -> NeuraxonMultiSphere:
    """v179 (v4.87) — CHC-aligned six-functional-sphere brain.

    Faithful Game-of-Life adaptation of an upcoming paper on the
    cross-battery dissociation of g. The paper's substrate is a
    six-sphere predictive-coding system whose spheres map to CHC broad
    abilities, with cross-sphere coupling κ and crystallised-sphere
    capacity λc identified as the dominant architectural levers for the
    recovered g. We mirror that here while preserving the Game-of-Life
    I/O contract (the game injects the 10 environmental inputs into the
    'sensory' sphere and reads the 7 behavioural outputs from the
    'motor' sphere — both keys are kept so the rest of the engine is
    unchanged and the legacy 3-sphere run stays bit-identical when this
    builder is not selected).

    Spheres (paper analogue in parentheses):
      sensory      — 10-channel environmental entry / thalamic relay
      visual       — Gv modality peer
      auditory     — Ga modality peer
      intero       — interoceptive peer (hunger / energy / temperature)
      assoc_fluid  — Gf+Gsm fluid-association coordinator
      assoc_cryst  — Glr+Gc crystallised store (capacity scaled by λc)
      motor        — Gs 7-channel behavioural readout

    Architectural levers (wired from the architecture in game_loop):
      cross_sphere_coupling  κ  — global multiplier on every inter-sphere
                                  link gain (paper's dominant transmission
                                  lever; κ→0 is the paper's headline lesion)
      cryst_capacity         λc — multiplies assoc_cryst hidden width +
                                  its plasticity (paper's other dominant
                                  lever)
      free_energy_beta       βF — multiplies link coherence_strength (how
                                  strongly CTC gates transmission ~ the
                                  paper's βF on the free-energy functional)
    """
    from .network import NeuraxonNetwork

    kappa = max(0.0, float(getattr(params, 'cross_sphere_coupling', 1.0)))
    lam_c = max(0.1, float(getattr(params, 'cryst_capacity', 1.0)))
    beta_f = max(0.0, float(getattr(params, 'free_energy_beta', 1.0)))

    brain = NeuraxonMultiSphere("NxEr Brain (chc6)")
    H = max(3, int(params.num_hidden_neurons))

    def _mk(name, n_in, n_hid, n_out, layer, depth, tags, desc):
        sp = _derive_sphere_params(
            params, network_name=name,
            num_input_neurons=n_in, num_hidden_neurons=max(3, n_hid),
            num_output_neurons=n_out)
        net = NeuraxonNetwork(sp)
        ins = [nn.id for nn in net.input_neurons]
        outs = [nn.id for nn in net.output_neurons]
        return net, ins, outs

    # --- sensory entry (keeps the 10-input game contract) ---
    s_net, s_in, s_out = _mk("Sensory Entry (CHC)", params.num_input_neurons,
                             max(3, H // 2), 6, "sensory", 0,
                             ["environmental", "thalamic_relay"],
                             "10-channel environmental entry / relay")
    brain.add_sphere("sensory", network=s_net,
                     interface=SphereInterface(
                         sensory_input_ids=s_in, relay_input_ids=s_in,
                         relay_output_ids=s_out, readout_output_ids=s_out),
                     layer_id="sensory", layer_depth=0,
                     modality_tags=["environmental"],
                     description="CHC sensory entry / thalamic relay")

    # --- modality peers: visual (Gv), auditory (Ga), intero ---
    peer_specs = [
        ("visual", "Visual Sphere (Gv)", ["visual", "Gv"]),
        ("auditory", "Auditory Sphere (Ga)", ["auditory", "Ga"]),
        ("intero", "Interoceptive Sphere", ["interoceptive"]),
    ]
    for sid, nm, tags in peer_specs:
        net, ins, outs = _mk(nm, 6, max(3, H // 2), 5, sid, 1, tags, nm)
        brain.add_sphere(sid, network=net,
                         interface=SphereInterface(
                             sensory_input_ids=ins[:2], relay_input_ids=ins[2:],
                             relay_output_ids=outs, readout_output_ids=outs),
                         layer_id="peer", layer_depth=1,
                         modality_tags=tags, description=nm)

    # --- assoc_fluid (Gf+Gsm coordinator) ---
    f_net, f_in, f_out = _mk("Assoc-Fluid (Gf+Gsm)", 8, max(4, H), 6,
                             "assoc_fluid", 2, ["multimodal", "Gf", "Gsm"],
                             "Fluid-association coordinator")
    brain.add_sphere("assoc_fluid", network=f_net,
                     interface=SphereInterface(
                         sensory_input_ids=f_in[:2], relay_input_ids=f_in[2:],
                         relay_output_ids=f_out, readout_output_ids=f_out),
                     layer_id="association", layer_depth=2,
                     modality_tags=["Gf", "Gsm"],
                     description="Fluid-association coordinator")

    # --- assoc_cryst (Glr+Gc store; capacity scaled by λc) ---
    cryst_h = max(4, int(round(H * lam_c)))
    c_net, c_in, c_out = _mk("Assoc-Cryst (Glr+Gc)", 8, cryst_h, 6,
                             "assoc_cryst", 2, ["Glr", "Gc"],
                             "Crystallised store (capacity λc)")
    brain.add_sphere("assoc_cryst", network=c_net,
                     interface=SphereInterface(
                         sensory_input_ids=c_in[:2], relay_input_ids=c_in[2:],
                         relay_output_ids=c_out, readout_output_ids=c_out),
                     layer_id="association", layer_depth=2,
                     modality_tags=["Glr", "Gc"],
                     description="Crystallised association store")

    # --- motor (Gs readout; keeps the 7-output game contract) ---
    m_net, m_in, m_out = _mk("Motor Sphere (Gs)", 6, max(3, H // 2),
                             params.num_output_neurons, "motor", 3,
                             ["action", "Gs"], "Motor readout")
    brain.add_sphere("motor", network=m_net,
                     interface=SphereInterface(
                         sensory_input_ids=m_in[:2], relay_input_ids=m_in[2:],
                         relay_output_ids=m_out[:3], readout_output_ids=m_out),
                     layer_id="motor", layer_depth=3,
                     modality_tags=["action"],
                     description="Motor readout (7 behavioural outputs)")

    # --- links: κ scales gain, βF scales coherence ---
    def _lp(kind, gain, coh, topo="dense", sparse=1.0, plast=0.0008, delay=1):
        return SphereLinkParameters(
            kind=kind, topology=topo, sparse_prob=sparse, delay_steps=delay,
            gain=max(0.0, gain * kappa),
            coherence_strength=max(0.0, min(1.0, coh * beta_f)),
            plasticity_rate=plast)

    def _link(a, b, kind, gain, coh, **kw):
        sa, sb = brain.spheres[a], brain.spheres[b]
        brain.connect_spheres(
            a, b,
            source_output_ids=sa.interface.relay_output_ids,
            target_input_ids=sb.interface.relay_input_ids,
            params=_lp(kind, gain, coh, **kw))

    # sensory → modality peers (feedforward)
    for sid in ("visual", "auditory", "intero"):
        _link("sensory", sid, "feedforward", 1.0, 0.25)
    # modality peers → both association spheres
    for sid in ("visual", "auditory", "intero"):
        _link(sid, "assoc_fluid", "feedforward", 0.9, 0.25)
        _link(sid, "assoc_cryst", "feedforward", 0.9, 0.25)
    # lateral cross-sphere coupling between the two association spheres
    # (this is precisely the link the paper severs in its κ=0 lesion)
    _link("assoc_fluid", "assoc_cryst", "lateral", 1.0, 0.30)
    _link("assoc_cryst", "assoc_fluid", "lateral", 1.0, 0.30)
    # association → motor (feedforward)
    _link("assoc_fluid", "motor", "feedforward", 1.0, 0.25)
    _link("assoc_cryst", "motor", "feedforward", 0.9, 0.25)
    # motor → assoc_fluid (feedback)
    _link("motor", "assoc_fluid", "feedback", 0.7, 0.20, topo="sparse",
          sparse=0.6, delay=2)
    # top-down: assoc_fluid → sensory (thalamic-like context)
    _link("assoc_fluid", "sensory", "thalamic_like", 0.6, 0.35)

    return brain


def build_brain(params: NetworkParameters) -> NeuraxonMultiSphere:
    """v179 (v4.87) — topology dispatcher.

    Selects the brain topology from ``params.sphere_topology``:
      'sensory_association_motor' (default) → build_default_multisphere
                                              (BIT-IDENTICAL to v178)
      'chc6'                                → build_chc_multisphere
    Any unknown value falls back to the legacy default, so a malformed
    architecture can never crash spawning.
    """
    topo = str(getattr(params, 'sphere_topology',
                       'sensory_association_motor'))
    if topo == 'chc6':
        return build_chc_multisphere(params)
    return build_default_multisphere(params)
