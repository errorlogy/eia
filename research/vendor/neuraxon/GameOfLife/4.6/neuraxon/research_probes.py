# Neuraxon Game of Life v.4.68 research_probes (Research Version):(Multi - Neuraxon 2.0 Compliant) Internal version 160
# Based on the Papers:
...
class HeritabilityTracker:
    ...
    v148 (v4.56) BUG FIX
    --------------------
    The previous implementation kept a list of (parent, child_fit) pairs and
    tried to dedup by removing earlier entries that matched BOTH the parent
    avg AND the current child_fit. Since child_fit changes every tick, the
    dedup never matched and the same child accumulated MANY pairs in the
    list — one per analytics tick, each with a different child_fit value.
    
    For correlation purposes this systematically biases toward NEGATIVE r:
    young children (just born) have low fitness while their parents had
    settled high fitness, and that pairing gets recorded once per tick over
    the child's whole lifetime. In a real run (game_id 105866379) this
    drove M10 to a stable -0.20 over 2400 ticks despite Aigarth working
    correctly otherwise.
    
    The fix: keep ONE pair per child in a dict keyed by child_id. The pair
    is overwritten every tick with the LATEST child fitness, so report()
    always uses the most recent snapshot of each living child.
    """
    def __init__(self):
        # v148 FIX: dict keyed by child_id, not list. One pair per child.
        self.pair_by_child: Dict[int, Tuple[float, float]] = {}
        # parent_avg recorded at the moment of birth — survives until death
        self.pending: Dict[int, float] = {}
    
    def register_birth(self, child_nxer, parents_fitness_avg: float):
        if child_nxer is None:
            return
        self.pending[child_nxer.id] = float(parents_fitness_avg)
    
    def update_alive(self, alive_nxers):
        """Snapshot fitness for every alive child whose parents we logged.
        Overwrites the existing pair for that child — never accumulates."""
        for a in alive_nxers:
            if a.id in self.pending:
                pavg = self.pending[a.id]
                fit = float(a.stats.fitness_score) if (a.stats is not None) else 0.0
                self.pair_by_child[a.id] = (pavg, fit)
        # Cap dict size to avoid unbounded growth across long sessions.
        # Discard the oldest 10% if we exceed 5000 entries — these correspond
        # to long-dead NxErs whose entries persist by design (a death does
        # NOT clear the pair, so we still see the parent→final-fitness link).
        if len(self.pair_by_child) > 5000:
            keys = list(self.pair_by_child.keys())
            for k in keys[:500]:
                del self.pair_by_child[k]
    
    def report(self) -> Dict[str, float]:
        n = len(self.pair_by_child)
        if n < HERITABILITY_MIN_PAIRS:
            return {'M10_heritability_r': 0.0, 'M10_heritability_pairs': float(n)}
        pairs = list(self.pair_by_child.values())
        ps = [p for p, _ in pairs]
        cs = [c for _, c in pairs]
        return {'M10_heritability_r': _safe_corr(ps, cs),
                'M10_heritability_pairs': float(n)}
