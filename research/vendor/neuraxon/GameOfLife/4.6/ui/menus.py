# Neuraxon Game of Life v.4.68 menus (Research Version):(Multi - Neuraxon 2.0 Compliant) Internal version 160
# Based on the Papers:
...
# Local imports
from .widgets import Slider, Checkbox  # v153: Checkbox added
...
def run_config_screen() -> Optional[Dict[str, Any]]:
    ...
    pygame.display.set_caption("Neuraxon Game Of Life v 4.68 (Research Version) By David Vivancos & Dr Jose Sanchez for Qubic Science - Configuration")
    ...
    # v153 (v4.61) — "Save full game logs to disk" checkbox.
    # When unchecked: logger STILL captures everything in memory, dashboard
    # works, KeyMetrics/Best NxErs auto-save still works. ONLY the
    # heavy full-history JSON dump in save_state() is skipped, saving
    # disk space for long runs (a 30-min run can produce 100+ MB logs).
    # When checked: full logs included as before.
    checkbox_y = start_y + len(param_specs) * slider_height + 15
    full_logs_checkbox = Checkbox(
        pygame.Rect(slider_start_x, checkbox_y, 20, 20),
        "Save full game logs to disk (KeyMetrics + Best NxErs auto-save regardless)",
        default=False
    )
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            full_logs_checkbox.handle_event(event)  # v153
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                if play_button_rect.collidepoint(event.pos):
                    ...
                    # v153 — pass the full-logs preference back to game_loop.
                    # Default False (saves disk for long runs); logs are
                    # always captured in memory regardless.
                    params["save_full_logs"] = full_logs_checkbox.get_value()
        ...
        title_surf = title_font.render("Neuraxon Game Of Life 4.5 (Research Version) - World Configuration", True, (235, 235, 240))
        screen.blit(title_surf, (screen.get_width() // 2 - title_surf.get_width() // 2, 50))
        # v4.5: In-game key-hint subtitle
        hint_font = pygame.font.SysFont("consolas", 14)
        hint = hint_font.render(
            "In-game keys:  SPACE = pause   V = toggle visuals   M = toggle NxEr singing (audio off by default)",
            True, (160, 200, 180),
        )
        screen.blit(hint, (screen.get_width() // 2 - hint.get_width() // 2, 90))
        for slider in sliders: slider.draw(screen, font)
        # v153 — draw the full-logs checkbox
        full_logs_checkbox.draw(screen, font)
