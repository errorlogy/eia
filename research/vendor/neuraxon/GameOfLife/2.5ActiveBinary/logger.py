...
class DataLogger:
    ...
    def _log_nxer_individual(self, tick: int, a: 'NxEr'):
        ...
        series['active_fraction'].append(sum(1 for s in states if s == 1) / len(states))   # Excitatory
        series['inactive_fraction'].append(sum(1 for s in states if s == -1) / len(states)) # Inhibitory
    ...
    def _log_tick_level2(self, tick: int, alive_nxers: list):
        ...
        self.time_series['active_fraction'].append(excitatory_frac)
        self.time_series['inactive_fraction'].append(inhibitory_frac)  # Renamed conceptually
