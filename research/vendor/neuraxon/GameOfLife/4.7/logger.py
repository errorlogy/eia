...
class DataLogger:
    ...
    def _log_tick_level2(self, tick: int, alive_nxers: list):
        ...
        try:
            ...
            # Sample 9 input neurons (first 3 NxErs × first 3 inputs)
            # every 100 ticks. Provides ground-truth for understanding
            # WHY mp values reach the magnitudes we observe.
            if (self._membrane_diag_enabled
                ...
                for a in alive_nxers[:3]:
                    if a.net is None or not a.net.input_neurons:
                        continue
                    for n in a.net.input_neurons[:3]:
                        ...
                        sampled += 1
                # Keep memory bounded — drop oldest rows past 20k
                if len(self._membrane_diag_rows) > 20000:
                    self._membrane_diag_rows = self._membrane_diag_rows[-15000:]
    ...
    def save_membrane_diagnostics(self, game_id: str,
        ...
        File format (one row per sampled neuron per sampled tick):
          tick  nxer_id  neuron_id  mp  adapt  autoreceptor
        ...
        try:
            with open(filename, 'w', encoding='utf-8') as f:
                ...
                f.write(f"# sampled every {self._membrane_diag_sample_every} ticks, "
                        f"first 3 input neurons of first 3 alive NxErs each sample\n")
                cols = ['tick', 'nxer_id', 'neuron_id', 'mp', 'adapt',
