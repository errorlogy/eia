# Neuraxon Game of Life v.4.92 neuron (Research Version):(Multi - Neuraxon 2.0 Compliant) Internal version 184
# Based on the Papers:
...
class Neuraxon:
    ...
    def __init__(self, neuron_id: int, neuron_type: NeuronType, params: NetworkParameters):
        ...
        # v172 (v4.80) — after-hyperpolarization. When > 0, mp is pulled
        # toward 0 by this fraction after every firing event (0→±1).
        # Default 0 = no reset (v171 behaviour). Combined with refractory
        # creates the paper's intended dynamics — see config.py docstring
        ...
        # v178 (v4.86) — make three previously-hardcoded neuron constants
        # configurable. Defaults equal the exact pre-v178 hardcoded values, so
        # any NetworkParameters lacking these fields (legacy / pure-default
        # runs) is BIT-IDENTICAL to v177. default.json already carries the
        # matching values (sensory_boost_scale=1.0, adaptation_tau_ticks=20.0,
        # autoreceptor_coefficient=0.1) — these were intended-but-unwired.
        self._sensory_boost_scale = float(getattr(params, 'sensory_boost_scale', 1.0))
        self._adaptation_tau_ticks = max(1.0, float(getattr(params, 'adaptation_tau_ticks', 20.0)))
        self._autoreceptor_coefficient = float(getattr(params, 'autoreceptor_coefficient', 0.1))
    ...
    def _update_autocorrelation(self):
        # v184 (v4.92) PERF: this runs for EVERY active neuron EVERY tick.
        # It previously called np.std() twice + np.corrcoef() on a ≤49-
        # element Python list; for arrays this small numpy's per-call
        # overhead (array alloc + dtype dispatch) is ~10-50× the actual
        # arithmetic. Replaced with an inline pure-Python population-std
        # gate + Pearson r using the SAME formulas, so the result is
        # identical up to float round-off — exactly the standard the
        # existing v4.52 PERF LUT optimisation already adopted. No
        # dynamics change: ths only feeds intrinsic_timescale, which is
        # then capped identically. The bare `except:` is also gone.
        sh = self.state_history
        n = len(sh)
        if n < 10:
            return
        # states_a = states[:-1], states_b = states[1:]  (lag-1 pairs)
        states = list(sh)
        m = n - 1
        a = states[:-1]
        b = states[1:]
        ...
        for i in range(m):
            ...
            var_b += db * db
            cov += da * db
        # np.std (population, ddof=0) gate: std = sqrt(var/m).
        # std < 1e-10  ⇔  var/m < 1e-20.  Compare without the sqrt.
        if var_a / m < 1e-20 or var_b / m < 1e-20:
            return
        denom = (var_a * var_b) ** 0.5
        if denom <= 0.0:
            return
        autocorr = cov / denom        # == np.corrcoef(a, b)[0, 1]
        if autocorr != autocorr:      # NaN guard (no bare except)
            return
    ...
    def update(self, synaptic_inputs: List[float], modulatory_inputs: List[float], external_input: float, neuromodulators: Dict[str, float], dt: float, global_osc: float, neighbor_phases: List[float] = None, receptor_activations: Dict = None):
        ...
        # v178 (v4.86) — was hardcoded `math.tanh(external_input)` (scale=1.0),
        # ignoring operating_ranges.sensory_boost_scale that the v152 comment
        # explicitly describes as scale*tanh(scale*x). Default 1.0 → identical.
        _sbs = self._sensory_boost_scale
        boosted_external = _sbs * math.tanh(_sbs * external_input)
        ...
        # adaptation_tau_ticks (default 20.0 → bit-identical to pre-v178).
        self.adaptation += dt / self._adaptation_tau_ticks * (-self.adaptation + adaptation_target)
        ...
        # v178 (v4.86) — was hardcoded 0.1; now operating_ranges.
        # autoreceptor_coefficient (default 0.1 → bit-identical to pre-v178).
        _arc = self._autoreceptor_coefficient
        theta1_eff = self.firing_threshold_excitatory + threshold_energy_mod - delta_theta_meta + delta_theta_homeo - _arc * self.autoreceptor
        theta2_eff = self.firing_threshold_inhibitory - threshold_energy_mod - delta_theta_meta + delta_theta_homeo + _arc * self.autoreceptor
        ...
        # v172 (v4.80) — additionally, after-hyperpolarization (mp reset)
        # on every firing event (0 → ±1). Pulls mp toward 0 by
        # `_post_spike_mp_reset` fraction. This is the critical mechanism
        # that lets state=0 persist NATURALLY (not just be forced briefly):
        # mp gets reset to rest band, so when refractory ends, mp is no
        # longer past threshold and state remains 0 until inputs rebuild
        # mp past threshold again. v171 found refract alone only achieved
        # 0.3% state=0 because mp stayed saturated.
        #
        # v174 (v4.82) — CRITICAL FIX: remove the `prev_trinary_state == 0`
        # gate. The v173 @3600s membrane diag showed state=0 was STILL only
        # 0.5% despite refract=9 + AHP + complement reset. Transition
        # analysis revealed why: 99% of firing activity is -1→-1 / +1→+1
        # (neurons LOCKED in a firing state) or direct -1↔+1 flips. The
        # `prev==0` gate meant refractory/AHP/complement-reset triggered on
        # only 4 of 777 firing events — the mechanisms were gated behind a
        # transition the pathology never takes. Biologically, EVERY spike is
        # followed by a refractory period + after-hyperpolarization,
        # regardless of the pre-spike state. A neuron whose input stays above
        # threshold should fire RHYTHMICALLY (spike → AHP → refractory rest →
        # mp rebuild → spike), not lock at +1 forever. So: trigger
        # refractory + AHP every tick the readout crosses threshold and the
        # neuron is not already refractory. With refract=0 AND psr=0
        # (defaults) this is bit-identical to v170 (both guards are False).
        if self.refractory_counter > 0:
            self.trinary_state = TrinaryState.NEUTRAL.value
            self.refractory_counter -= 1
        elif readout > theta1_eff:
            self.trinary_state = TrinaryState.EXCITATORY.value
            # v174 — every spike triggers refractory + AHP (no prev==0 gate)
            if self._refractory_period > 0:
                self.refractory_counter = self._refractory_period
            if self._post_spike_mp_reset > 0:
                keep = 1.0 - self._post_spike_mp_reset
                self.membrane_potential *= keep
                self.state_tilde *= keep
                self.complement_h *= keep
        elif readout < theta2_eff:
            self.trinary_state = TrinaryState.INHIBITORY.value
            # v174 — every spike triggers refractory + AHP (mirror, no gate)
            if self._refractory_period > 0:
                self.refractory_counter = self._refractory_period
            if self._post_spike_mp_reset > 0:
                keep = 1.0 - self._post_spike_mp_reset
        ...
        if abs(self.autoreceptor) > 0.1 and _ll >= 2:
            threshold_effect = -self._autoreceptor_coefficient * self.autoreceptor  # v178: match theta computation
        ...
        if prev_state != self.trinary_state and _ll >= 2:
            ...
            autoreceptor_contrib = -self._autoreceptor_coefficient * self.autoreceptor  # v178: match theta computation
