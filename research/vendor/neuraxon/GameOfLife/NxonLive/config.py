# Neuraxon Game of Life v.4.92 config (Research Version):(Multi - Neuraxon 2.0 Compliant) Internal version 184
# Based on the Papers:
...
class NetworkParameters:
    ...
    # UPDATED v4.5: 6 original + DayNight, Temperature, Proprioception, Song = 10 inputs
    ...
    #   9: Song/Hearing (-1=dissonant song nearby, 0=no song, 1=harmonic song nearby)  [NEW v4.5]
    num_input_neurons: int = 10
    num_hidden_neurons: int = 10 
    # UPDATED v4.5: 5 original + Resting + Sing = 7 outputs
    ...
    #   6: Sing (-1=silent, 0=hum softly, 1=sing full voice)  [NEW v4.5]
    num_output_neurons: int = 7
    ...
    # v178 (v4.86) — three constants the neuron model previously hardcoded,
    # now configurable so the architecture's operating_ranges/neural sections
    # actually take effect. Defaults equal the EXACT pre-v178 hardcoded values
    # (neuron.py: dt/20.0, 0.1*autoreceptor, tanh(x) i.e. scale 1.0) and match
    # architectures/default.json, so legacy / pure-default runs are
    # bit-identical to v177. See CHANGELOG_v178.
    ...
    # by the chc6 builder, and fitness_g_weight=0.0 means g is measured/logged
    # but never alters selection unless the researcher opts in. See
    # CHANGELOG_v179 / docs/G_FACTOR_METHODOLOGY.md.
    sphere_topology: str = "sensory_association_motor"  # 'chc6' = paper's 6-sphere
    cross_sphere_coupling: float = 1.0       # κ — inter-sphere link gain multiplier
    cryst_capacity: float = 1.0              # λc — assoc_cryst width/plasticity scale
    free_energy_beta: float = 1.0            # βF — link coherence (CTC) strength scale
    fitness_g_weight: float = 0.0            # opt-in: weight of population-g in fitness
    ...
    # v148 (v4.56) FIX (M6): Bumped from 0.003 to 0.020 to restore biologically
    # plausible spontaneous firing. The Neuraxon v2.0 paper §V calls
    # self-generated activity a "cornerstone" feature; M6 healthy band is
    # [0.10, 0.45] (= 10-45% of spikes spontaneous). v129 reduced this to
    # 0.003 trying to fix M1 excitatory dominance, but M1=0.62 stayed too
    # high anyway (the real cause of M1 was elsewhere — see v131/v132 notes)
    # while M6 collapsed to 0.000 every sample.
    #
    # In real runs we observed NxErs settling into a frozen attractor and
    # starving when no immediate stimulus was present (v146 user report:
    # "Nxers tend to stop moving and die"). The mechanism: with M6=0 the
    # network is purely reactive — no endogenous drive to keep exploring.
    # 0.020 = 2% per neuron per tick gives ~5 spontaneous spikes per neuron
    # per second of game time at 60 fps, well within the cortical norm
    # (~1-10 Hz spontaneous rate in vivo, Sanchez-Vives & McCormick 2000).
    #
    # If this overshoots M1 again, the right next lever is the membrane
    # decay (resting_potential_decay) or the firing thresholds — NOT this.
    spontaneous_firing_rate: float = 0.020  # v148: Up from 0.003 (M6 fix)
    ...
    # v172 (v4.80) — post-spike membrane reset (after-hyperpolarization).
    # When > 0, after every firing event (0 → ±1) the membrane potential
    # is pulled toward 0 by this fraction. 0.0 = no reset (v171 behaviour
    # where mp persists past threshold even during refractory). 1.0 = full
    # reset to mp=0. Biological neurons exhibit after-hyperpolarization
    # (AHP) — mp drops sharply post-spike, often below resting potential.
    # Refractory alone doesn't create a true 0 buffer because mp stays
    # past threshold and the neuron re-fires immediately when refractory
    # ends. With reset, mp drops back to rest band and state STAYS at 0
    # naturally until inputs push it past threshold again. Combined with
    # refractory_period_ticks this restores the paper's intended trinary
    # dynamics: spike → reset → quiescent → slow rebuild → spike. Both
    # `membrane_potential` and `state_tilde` (CTSN complement) are reset.
    post_spike_mp_reset: float = 0.0
    ...
    # =================================================================
    # VOICE / SONG / AUDIO PARAMETERS (NEW v4.5)
    # =================================================================
    # BIOINSPIRED: Tonal vocalisations couple social behaviour to an
    # acoustic dimension. See simulation/voice.py for the consonance
    # model grounded in Plomp–Levelt / Kameoka–Kuriyagawa psychoacoustics.

    voice_enabled: bool = True                 # Global on/off for the voice subsystem
    voice_hearing_radius: int = 4              # Square radius of the "song near me" scan
    voice_max_listeners: int = 6               # Cap nearby-voices list cost per tick
    voice_clan_affinity_weight: float = 0.35   # Tone-similarity bias added to movement
    voice_choir_nearby_threshold: int = 3      # NxErs needed to earn choir serotonin boost
    voice_choir_serotonin_boost: float = 0.02  # Per-tick boost when choir conditions met
    voice_sing_food_threshold: float = 0.20    # Fraction of StartFood below which singing suppresses (sick/starving don't sing)
    voice_food_pitch_semitones: float = 3.0    # Max pitch elevation when fully fed
    voice_discovery_pitch_semitones: float = 2.0  # Transient spike on new food
    voice_discovery_duration_ticks: int = 30   # Length of that transient
    voice_inherit_novel_tones: int = 2         # # of fresh tones per child (rest = 4 inherited)
    voice_mate_tone_exchange: bool = True      # Partners copy 1 tone from each other at mating

    # Audio (rendering only — OFF by default, toggled with M key in-game)
    audio_enabled_default: bool = False
    audio_min_zoom: float = 8.0                # Below this, audio is silent
    audio_max_screen_dist: float = 450.0       # Pixels; fade to zero here
    audio_max_voices: int = 8                  # Hard ceiling on channels
    audio_max_new_per_frame: int = 4           # Throttle for cold-cache inits
