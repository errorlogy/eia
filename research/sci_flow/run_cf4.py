"""Run CF-4 / internal-state reset suite (M-CF4 / C2 path)."""
...
from eia.cf4 import CF4SeedResult, run_seed, summarize  # noqa: E402


def _job(payload: tuple[int, str]) -> CF4SeedResult:
...
def main() -> int:
    ...
    out = Path(__file__).resolve().parent / "cf4_results.json"
    out.write_text(
        json.dumps(
            {
                "summary": summary,
