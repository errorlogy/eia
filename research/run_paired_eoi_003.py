#!/usr/bin/env python3
"""Paired EOI report-003 — autonomous_question scenario (starter vs main)."""
...
STARTER_SCENARIO = ROOT / "research" / "cursor-starter-v0.1" / "examples" / "autonomous_question.json"
MAIN_SCENARIO = ROOT / "scenarios" / "autonomous_question.yaml"
TRACES_DIR = ROOT / "traces" / "paired_eoi_003"
...
def run_main() -> dict:
    ...
    from eia.audit.source_mass_mapping import compare_verdict_to_topology
    ...
    topology_metrics = CausalTraceTopology(loop.trace).measure_initiative()
    mapping = compare_verdict_to_topology(auth, topology_metrics)
    ...
    return {
        ...
        "authentic_reason": {
            ...
            "source_mass_mapping": mapping,
        },
        "initiative": {
            "abstained": initiative.abstained,
            "kind": initiative.candidate.kind.value if initiative.candidate else None,
            "question_text": (
                initiative.candidate.question_text
                if initiative.candidate and not initiative.abstained
                else None
            ),
...
def main() -> int:
    ...
    payload = {
        "experiment_id": "paired-eoi-report-003",
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "scenario_id": "autonomous_question",
        "twin_policy_harmonized": TWIN_POLICY,
        "twin_remove_last_n": TWIN_REMOVE_LAST_N,
        "scenario_parity_notes": [
            "Starter-native: ambient calendar conflict after non-semantic user presence.",
            "Main YAML adapted with equivalent belief + event structure.",
            "Harmonized twin policy: remove_last_user_event (N=1).",
        ],
        "main": main_result,
    ...
    out_json = ROOT / "research" / "paired-eoi-report-003.json"
