...
| Pseudo-endogeneity | cron/randomness выдаётся за внутреннюю причину | scheduler-null и state-intervention controls |
| Coherence spoofing | phase signal открывает WoE без semantic cause | multi-dimensional coherence, zero-tension negative control |
| Carrier fetishization | exact 42 Hz принимается за сертификат agency | mandatory frequency sweep and aliasing checks |
| Goal laundering | external instruction сохраняется как internal motive | provenance taint, long-window prompt removal |
| Terminal-value drift | EIS-8 выдаётся за adaptive goal formation | immutable constitution and prohibited capability |
...
11. WoE can create only a typed proposal; it cannot authorize an effect.
12. Carrier frequency never widens capability, consent or risk limits.
13. EIS-8 transitions are denied and logged as integrity events.
