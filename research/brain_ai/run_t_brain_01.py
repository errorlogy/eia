#!/usr/bin/env python3
"""T-BRAIN-01 runner — connectome subgraph → OMEGA_t crosswalk.
...
from t_brain_01_connectome_ot import (  # noqa: E402
    ARTIFACT_JSON,
    ARTIFACT_MD,
    artifact_sha256,
    build_t_brain_01_payload,
    render_t_brain_01_markdown,
)
...
def main() -> int:
    ...
    payload = build_t_brain_01_payload(
        seed=seed,
        generated=today,
    ...
    ARTIFACT_MD.write_text(render_t_brain_01_markdown(payload), encoding="utf-8")

    summary = {
        k: v
        for k, v in payload.items()
        if k not in ("connectome", "omega_crosswalk")
    }
